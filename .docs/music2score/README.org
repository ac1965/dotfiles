...
* Overview
本プロジェクトは、単一の音源ファイルから以下を自動生成するパイプラインです。
...
共通コード・依存関係は =env/common/= に 1 つだけ出力し、
...
#+begin_src python :tangle env/common/scripts/ensemble_stems.py :shebang "#!/usr/bin/env python3"
  ...
  def load_stereo(path: Path) -> tuple[np.ndarray, int] | tuple[None, None]:
      if not path.exists():
          return None, None
      y, sr = sf.read(path)
      if y.ndim == 1:
  ...
  def main() -> None:
      ...
      ap.add_argument("--stem-root", required=True, help="root where model/track/stem.wav lives")
      ap.add_argument("--track", required=True, help="basename of track (without extension)")
...
#+begin_src makefile :tangle env/macOS/Makefile
  ...
  # MuseScore 4.app の CLI パス（必要に応じて変更）
...
** Dockerfile（ビルドコンテキストは project-root）
