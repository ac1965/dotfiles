...
#+PROPERTY: header-args :noweb :results silent :exports code :mkdirp yes :padline no :tangle no
...
#+begin_src sh :tangle scripts/process.sh :shebang "#!/usr/bin/env bash"
  ...
  for m in "${MODEL_ARR[@]}"; do
    echo "[demucs] model=$m"
    OUT_SUBDIR="$STEM_ROOT/$m"            # モデル別ディレクトリを指定
    mkdir -p "$OUT_SUBDIR"                # 例: out/stems/htdemucs_6s/
    demucs -n "$m" --overwrite -o "$OUT_SUBDIR" "$WAV_IN" >/dev/null 2>&1
  done
