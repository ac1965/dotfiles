...
print(json.dumps({
    ...
    # gpt-oss:20bではthinkingチャンネルへの出力が抑制されない挙動を
    # 実機で確認した(2026-09-21。Ollama側がgpt-oss+/api/generateの
    # 組み合わせでthinkを完全には尊重していない可能性がある)。指定自体は
    # 無害なので残しつつ、根本対策としてnum_predictを増やし、thinkingが
    # 数百トークンかかっても最終回答を書き切れる余裕を持たせる。
    "think": False,
