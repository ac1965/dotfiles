...
#
# 生成メッセージが規約に従わない事故について(2026-09-19〜21に実機で確認):
#   ブログ記事(content/post/配下)のような、日本語の文章がまるごと含まれる
#   diffをqwen3:14bに渡すと、diffをコミット要約の対象データとしてではなく
#   「自分への話しかけ」と誤解し、規約を無視した感想文・レビュー文を生成
#   することがあった(2回とも --commit がそのまま規約外のメッセージで
#   コミットしてしまった)。対策として (1) diffを <<<DIFF_START>>> /
...
print(json.dumps({
    ...
    "stream": False,
    # num_predict でレスポンス長に上限を設ける。コミットメッセージは本来
    # 数行に収まるはずで、モデルが応答をハングさせず律義に長文を生成し
    # 続ける事故(2026-09-21に実機で確認: 1500トークン超の雑感文を生成し、
    # 数分待たされた)を早期に打ち切るための安全弁。
