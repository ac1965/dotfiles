...
#   5) macOS システム環境設定(defaults)の復元(あれば) ... .local/bin/defaults.zsh restore-all
#   6) Emacs のビルド                                 ... .local/bin/build-emacs-macos.sh
#
...
readonly USAGE="usage: ${0:t} [-n|--dry-run] [--skip-brew] [--skip-dotfiles] [--skip-private] [--skip-defaults] [--skip-emacs] [-h|--help]"

integer DRYRUN=0 SKIP_BREW=0 SKIP_DOTFILES=0 SKIP_PRIVATE=0 SKIP_DEFAULTS=0 SKIP_EMACS=0
for arg in "$@"; do
  case $arg in
    -n|--dry-run)     DRYRUN=1 ;;
...
readonly -i DRYRUN SKIP_BREW SKIP_DOTFILES SKIP_PRIVATE SKIP_DEFAULTS SKIP_EMACS
(( DRYRUN )) && print -- "-- DRY RUN: 副作用のあるコマンドは実行せず表示のみ --"

log_step() { print -- "\n==> $1"; }

log_step "1/6 Xcode Command Line Tools / Homebrew / iTerm2"
if (( SKIP_BREW )); then
...
log_step "2/6 Brewfile 一括インストール"
if (( SKIP_BREW )); then
  print -- "  skip (--skip-brew)"
elif (( DRYRUN )); then
...
log_step "4/6 private アーカイブの復号・配置"
...
if (( SKIP_DEFAULTS )); then
  ...
  zsh "${REPO_ROOT}/.local/bin/defaults.zsh" restore-all
fi

log_step "6/6 Emacs ビルド"
if (( SKIP_EMACS )); then
